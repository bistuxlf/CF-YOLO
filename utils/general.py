...
# def sf_nms(bboxes, scores, iou_thresh=0.5,sigma=0.5,score_threshold=0.25):
...
def sf_nms(bboxes, scores, iou_thresh=0.5, sigma=0.5, score_threshold=0.25, aspect_ratio_target=1.2, aspect_ratio_weight=0.1):
    order = scores.argsort(descending=True).to(bboxes.device)
    keep = []
    ...
    while order.numel() > 1:
        ...
        if idx.numel() > 0: 
            ...
            # 计算长宽比
            bboxes_selected = bboxes[order[idx+1]]
            widths = bboxes_selected[:, 2] - bboxes_selected[:, 0]
            heights = bboxes_selected[:, 3] - bboxes_selected[:, 1]
            aspect_ratios = widths / heights
            
            # 计算长宽比惩罚系数
            aspect_ratio_penalty = torch.exp(-aspect_ratio_weight * torch.abs(aspect_ratios - aspect_ratio_target))
            
            # 结合长宽比惩罚的newScores
            newScores = torch.exp(-torch.pow(iou, 2) / sigma) * aspect_ratio_penalty
            scores[order[idx+1]] *= newScores
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        i = sf_nms(boxes, scores, iou_thres)
